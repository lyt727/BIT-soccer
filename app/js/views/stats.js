...
function resultPreviewCard(res, low) {
  ...
  const block = el('div', { class: 'card mt8' },
    ...
    el('div', { class: 'kv' },
      ...
      // 日期/场地与工作人员不在裁判报告范围内，报告上真写了才显示，避免误以为漏识别
      (m.date || m.time || m.venue) ? el('dt', {}, '比赛信息') : null,
      (m.date || m.time || m.venue) ? el('dd', {}, matchInfoText(m)) : null,
      (m.staff && Object.values(m.staff).some(Boolean)) ? el('dt', {}, '工作人员') : null,
      (m.staff && Object.values(m.staff).some(Boolean)) ? el('dd', {}, staffRolesText(m.staff)) : null,
